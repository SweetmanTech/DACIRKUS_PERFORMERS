...
const SectionLayout: React.FC<LayoutProps> = ({ containerClassName, className, children }) => {
  ...
  return (
    <div className={`${containerClassName} relative`} ref={containerRef}>
      <div ref={cursorRef} className="hidden rounded-full w-[122px] h-[122px] z-[30] absolute border-[1px] border-[black] dark:border-[white] pointer-events-none p-1">
        <div className="border-[1px] border-[white] w-[100%] h-[100%] rounded-full"></div>
      </div>
      
      <div className={`relative ${className} bg-center bg-cover bg-no-repeat dark:!block hidden`}>
      <div ref={darkImageRef} className={`absolute ${className} bg-center bg-cover bg-no-repeat z-[5] w-full h-full opacity-[0]`}></div>
      ...
      <div className="dark:hidden block">
        <div ref={whiteImageRef} className={`absolute ${className} bg-center bg-cover bg-no-repeat z-[5] w-full h-full opacity-[0]`}></div>
        <div className="relative z-[6]">
          <Container>
            {children}
          </Container>
