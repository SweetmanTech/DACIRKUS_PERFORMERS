...
const AvatarGroup: React.FC<AvatarGroupProps> = ({children, count, size}) => (
    ...
    >
        {children && children.map((child: any, index: number) => (
            <div className={`relative z-[${index}] w-[${styles.sizes[size]}px] h-[${styles.sizes[size]}px]`}
                style={{
                    transform:  `translateX(${(styles.sizes[size] - styles.gap[size]) * index}px) translateY(-${styles.sizes[size] * index}px)`
                }}
                key={index}
            >
                {child}
